...
**Se tireoide alterada → Veja seção "DESCARTAR HIPERTIREOIDISMO"**
...
→ Tireoide descartada como causa
→ Continue para identificação dos 5 tipos de ansiedade
→ Protocolo nutricional seguro para aplicar
...
**Erro #1 mais comum:**
Achar que é "só psicológico" e não investigar tireoide, glicemia, intestino, ou deficiências nutricionais.
...
**Resultado esperado:**
Redução de 40-60% dos sintomas em 2-4 semanas.
...
**Fisiopatologia:**
Eixo HPA cronicamente ativado → cortisol elevado → hipervigilância → GABA insuficiente → ansiedade persistente.
...
**Fisiopatologia:**
Hiperventilação → ↓ CO₂ → alcalose respiratória → vasoconstrição cerebral → sintomas físicos → interpretação catastrófica → mais ansiedade (ciclo).
...
**Fisiopatologia:**
Serotonina baixa + amígdala hiperativa → medo excessivo de julgamento → evitação → reforço negativo.
...
**Fisiopatologia:**
Desregulação autonômica, hormonal, ou metabólica → sintomas físicos de ansiedade **sem** preocupação mental.
...
**Fisiopatologia:**
Intestino permeável (leaky gut) + disbiose → inflamação → citocinas → atravessam barreira hematoencefálica → ansiedade.
...
O **[Protocolo Mestre (Shot dos Campeoes)](/pt/docs/protocolo-mestre/)** contem componentes que atuam diretamente na ansiedade:
